...
OSSL_CMP_CTX_get0_last_senderNonce,
...
OSSL_CMP_CTX_create() allocates and initializes an OSSL_CMP_CTX structure to
default values.
...
OSSL_CMP_CTX_get0_recipNonce() gets the recipient nonce of the given context.
...
OSSL_CMP_CTX_get0_last_senderNonce() gets the sender B<nonce> of the last 
message sent. NULL if not set or on function parameter error.
...
OSSL_CMP_CTX_create() returns a pointer to an initialized OSSL_CMP_CTX structure.
